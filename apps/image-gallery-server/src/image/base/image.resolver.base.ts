...
export class ImageResolverBase {
  ...
  async createImage(@graphql.Args() args: CreateImageArgs): Promise<Image> {
    return await this.service.createImage({
      ...
      data: {
        ...
        gallery: args.data.gallery
          ? {
              connect: args.data.gallery,
            }
          : undefined,
      },
    });
  ...
  @graphql.ResolveField(() => Gallery, {
    nullable: true,
    name: "gallery",
  })
  async getGallery(@graphql.Parent() parent: Image): Promise<Gallery | null> {
    const result = await this.service.getGallery(parent.id);
